...
{
    return QCoreApplication::translate("greenrenovatedbonds-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in greenrenovatedbonds.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in greenrenovatedbonds.qrc)
...
/** Class encapsulating greenrenovatedbonds Core startup and shutdown.
...
/** Main greenrenovatedbonds application object */
...
#include "greenrenovatedbonds.moc"
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // greenrenovatedbonds: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. greenrenovatedbonds Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(greenrenovatedbonds);
    Q_INIT_RESOURCE(greenrenovatedbonds_locale);
    ...
    /// 6. Determine availability of data directory and parse greenrenovatedbonds.conf
    ...
    // greenrenovatedbonds: links repeatedly have their payment requests routed to this process:
