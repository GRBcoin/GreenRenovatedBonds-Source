...
//greenrenovatedbonds only features
...
const char * const BITCOIN_CONF_FILENAME = "greenrenovatedbonds.conf";
const char * const BITCOIN_PID_FILENAME = "greenrenovatedbondsd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "greenrenovatedbonds" is a composite category enabling all greenrenovatedbonds-related debug output
                if(ptrCategory->count(std::string("greenrenovatedbonds"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\greenrenovatedbonds
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\greenrenovatedbonds
    // Mac: ~/Library/Application Support/greenrenovatedbonds
    // Unix: ~/.greenrenovatedbonds
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "greenrenovatedbonds";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/greenrenovatedbonds";
#else
    // Unix
    return pathRet / ".greenrenovatedbonds";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty greenrenovatedbonds.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override greenrenovatedbonds.conf
