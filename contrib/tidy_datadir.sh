...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete greenrenovatedbonds database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no greenrenovatedbonds datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old greenrenovatedbonds datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected greenrenovatedbonds 0.7 datadir."
    ;;
  3)
    echo "Detected greenrenovatedbonds pre-0.8 datadir."
    ;;
  4)
    echo "Detected greenrenovatedbonds 0.8 datadir."
    ;;
esac
